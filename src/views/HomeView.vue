...
<template>
  <section>
    ...
    <div v-else>
      <div v-if="lottoStore.errorMsg" class="text-center">
        {{ lottoStore.errorMsg }}
      </div>
      <div v-else class="lotto-info">
        <h1>
          ...
          <span class="draw-date"
            >({{ lottoStore.formattedDrawDate }} 추첨)</span
          >
        </h1>
        <v-list lines="two">
          <v-list-item
            title="총 판매금액"
            :subtitle="lottoStore.formattedTotalSellAmount"
          />

          <v-list-item
            title="1등 당첨금액"
            :subtitle="lottoStore.formattedTotalPrize"
          />

          <v-list-item
            title="1등 당첨게임 수"
            :subtitle="lottoStore.firstPrizeWinners"
          />

          <v-list-item
            title="1게임당 당첨금액"
            :subtitle="lottoStore.prizePerGame"
          />
        </v-list>
