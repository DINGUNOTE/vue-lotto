...
export const useLottoStore = defineStore('lotto', () => {
  ...
  const mostFrequentNumbers = ref([]) // 최신 10회차 당첨번호 중 가장 많이 나온 번호
  const leastFrequentNumbers = ref([]) // 최신 10회차 당첨번호 중 가장 적게 나온 번호
  const maxFrequency = ref(0) // 최신 10회차 당첨번호 중 가장 많이 나온 번호의 빈도수
  const minFrequency = ref(0) // 최신 10회차 당첨번호 중 가장 적게 나온 번호의 빈도수
  ...
  // 최신 10회차 당첨번호 중 가장 많이 나온 번호와 가장 적게 나온 번호 찾기
  const getMostAndLeastFrequentNumbers = async () => {
    const frequency = await fetchLast10Draws()
